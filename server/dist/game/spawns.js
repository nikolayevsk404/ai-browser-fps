...
export function getSpawn(team, index) {
    const spawns = MAP_SPAWNS[team];
    return { ...spawns[index % spawns.length] };
}
export function getDeathmatchSpawn(players, respawningPlayer) {
...
function scoreSpawn(spawn, players, respawningPlayer) {
    let nearestEnemy = 40;
    let nearestAlly = 40;
...
function intersectsCollider(x, z) {
    return MAP_COLLIDERS.some((collider) => x + playerRadius > collider.minX && x - playerRadius < collider.maxX && z + playerRadius > collider.minZ && z - playerRadius < collider.maxZ);
}
