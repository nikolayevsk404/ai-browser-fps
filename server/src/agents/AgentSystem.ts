...
const roleCycle: AgentRole[] = ["entry", "support", "lurker", "sniper", "anchor"];
const roleIndex = Object.fromEntries(roleCycle.map((role, index) => [role, index])) as Record<AgentRole, number>;
...
const roleNavTargets: Record<AgentRole, Record<TeamId, string>> = {
...
const patrolRoutes: Record<TeamId, Record<AgentRole, string[]>> = {
  CT: {
...
const coverNavByTeam: Record<TeamId, string[]> = {
  CT: ["ct-spawn", "apartments", "sawmill", "forest-south", "river-bridge"],
  TR: ["tr-spawn", "cabins-east", "market", "forest-north", "river-bridge"]
};

const navPositions: Record<string, Vector3> = Object.fromEntries(MAP_NAV_POINTS.map((point) => [point.id, point.position]));
const navNeighbors: Record<string, string[]> = Object.fromEntries(MAP_NAV_POINTS.map((point) => [point.id, point.neighbors]));

export class AgentSystem {
  createAgents(players: PlayerState[]): AgentState[] {
    const teamIndexes: Record<TeamId, number> = { CT: 0, TR: 0 };
  ...
  update(state: GameState, now: number): BotCombatResult[] {
    if (state.match.phase !== "running" || state.match.roundPhase !== "live") {
      return [];
    }

    const combatResults: BotCombatResult[] = [];
    const comms: AgentCommunication[] = [];
...
function createAgent(player: PlayerState, role: AgentRole): AgentState {
...
function perceive(state: GameState, player: PlayerState, agent: AgentState, now: number): AgentState["perception"] {
  ...
  const allyUnderFire = state.combatEvents.some(
    (event) => event.type === "ally_under_fire" && event.team === player.team && now - event.at < 1800 && distance(player.position, event.position) <= nearbyRange
  );
...
function updateCombatEvents(state: GameState, agent: AgentState, player: PlayerState, events: CombatEvent[], now: number): void {
...
function transitionBrain(state: GameState, agent: AgentState, player: PlayerState, now: number): void {
  ...
  let next: BotBrainState = brain.state;
...
function navigate(state: GameState, player: PlayerState, agent: AgentState, now: number): void {
  const brain = agent.brain;
  ...
  const speed =
    brain.state === "Retreat" || brain.state === "Chase"
      ? 1.28
...
function tryBotShoot(state: GameState, agent: AgentState, player: PlayerState, now: number): BotCombatResult | null {
  const brain = agent.brain;
  const combatStates: BotBrainState[] = ["Engage", "StrafeCombat", "Chase", "PushEnemy", "TakeCover", "Search", "Investigate"];
...
function createCommunication(agent: AgentState): string | null {
  if (agent.perception.visibleEnemyIds.length > 0) {
...
function updateMemory(agent: AgentState, player: PlayerState, now: number): void {
  if (agent.perception.heardShotAt) {
...
function findWoundedEnemy(state: GameState, agent: AgentState): PlayerState | null {
  return (
    state.players.find(
      (candidate) =>
        ...
        distance(candidate.position, agent.memory.lastKnownPlayerPosition) < 12
    ) ?? null
  );
}

function pickCover(agent: AgentState): string {
...
function getPatrolTarget(agent: AgentState): string {
...
function targetPriority(player: PlayerState, enemy: PlayerState): number {
...
function getSeparation(state: GameState, player: PlayerState): Vector3 {
...
function moveBotWithSlide(player: PlayerState, next: Vector3): Vector3 | null {
...
function checkStuck(agent: AgentState, player: PlayerState, now: number, arrived: boolean): void {
...
function randomEscapeDirection(position: Vector3): Vector3 {
...
function perpendicular(target: Vector3, origin: Vector3, dir: -1 | 1): Vector3 {
...
function nearestNavPoint(position: Vector3): string {
  return Object.entries(navPositions).reduce(
    (best, [id, navPosition]) => {
      const navDistance = distance(position, navPosition);
      return navDistance < best.distance ? { id, distance: navDistance } : best;
    },
    { id: "town-square", distance: Infinity }
  ).id;
}

function findPath(start: string, goal: string): string[] {
  ...
  const fScore = new Map<string, number>([[start, distance(navPositions[start], navPositions[goal])]]);
...
function score(scores: Map<string, number>, id: string): number {
  return scores.get(id) ?? Infinity;
}

function hasLineOfSight(a: Vector3, b: Vector3): boolean {
  return !MAP_COLLIDERS.some((collider) =>
    segmentIntersectsBox(a.x, a.z, b.x, b.z, collider.minX - botRadius, collider.minZ - botRadius, collider.maxX + botRadius, collider.maxZ + botRadius)
  );
}

function intersectsCollider(x: number, z: number): boolean {
  return MAP_COLLIDERS.some(
    (collider) => x + botRadius > collider.minX && x - botRadius < collider.maxX && z + botRadius > collider.minZ && z - botRadius < collider.maxZ
  );
}

function segmentIntersectsBox(x1: number, z1: number, x2: number, z2: number, minX: number, minZ: number, maxX: number, maxZ: number): boolean {
...
function velocityAnimation(velocity: Vector3, yaw: number): PlayerState["animation"] {
...
function dampVelocity(velocity: Vector3): Vector3 {
...
function createEmptyPerception(): AgentState["perception"] {
  return {
    visibleEnemyIds: [],
    heardShotAt: null,
